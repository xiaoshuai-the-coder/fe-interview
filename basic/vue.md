...
这份笔记以“面试可讲清楚”为目标，优先整理高频原理点与可落地的回答结构。

### 响应式原理（占位）

这里后续可以补：依赖收集（track）/派发更新（trigger）、Proxy vs defineProperty 差异、effect/scheduler、计算属性与 watch 的实现差别等。

### Diff 算法

#### Vue 2：双端比较

Vue 2 在更新同级子节点时，会用**双端指针**对老/新 vnode 列表做比较，常见流程可以概括为：
...
1. 当 `key` 相同且 `tag` 相同，节点可以被复用，从而减少不必要的创建与销毁，提高更新效率。
2. 在 diff 过程中可以通过 key 建立映射，降低查找成本，避免在列表里反复线性扫描。

如果不写 `key`（或 `key` 不稳定），框架可能倾向于“就地复用”，从而在列表增删改时出现状态错位或不必要的更新。因此 `key` 应该**稳定且唯一**，通常优先使用数据的业务 ID，尽量避免使用 index 或随机数。
...
Vue 3 相比 Vue 2 的核心思路是：**能在编译期确定的信息尽量前置**，运行时就少做工作，典型优化点包括：

1. **`patchFlag`**：标记动态内容类型，更新时不需要对整棵子树做全量比较。
2. **最长递增子序列（LIS）**：在 keyed diff 场景下，通过 LIS 找到“无需移动”的节点集合，从而**最小化 DOM 移动次数**。
3. **静态提升（hoistStatic）**：静态节点提升为常量，避免重复创建 vnode。
4. **事件处理函数缓存**：减少不必要的函数创建与 props 变更带来的更新。
